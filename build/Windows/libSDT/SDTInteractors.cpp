...
#define MAX_ITERATIONS 50
	...
	__declspec(dllexport) SDTInteractor *SDTInteractor_new() {
		SDTInteractor *x;
	...
	__declspec(dllexport) void SDTInteractor_free(SDTInteractor *x) {
		free(x);
	}

	__declspec(dllexport) void SDTInteractor_setFirstResonator(SDTInteractor *x, SDTResonator *p) {
		x->obj0 = p;
	}

	__declspec(dllexport) void SDTInteractor_setSecondResonator(SDTInteractor *x, SDTResonator *p) {
		x->obj1 = p;
	}

	__declspec(dllexport) void SDTInteractor_setFirstPoint(SDTInteractor *x, long l) {
		x->contact0 = l;
	}

	__declspec(dllexport) void SDTInteractor_setSecondPoint(SDTInteractor *x, long l) {
		x->contact1 = l;
	}

	__declspec(dllexport) double SDTInteractor_computeForce(SDTInteractor *x) {
		double f, h, w, f0, f1;
		int count;
	...
	__declspec(dllexport) void SDTInteractor_dsp(SDTInteractor *x, double f0, double v0, double s0,
	...
	__declspec(dllexport) double SDTImpact_MarhefkaOrin(SDTInteractor *x) {
		SDTImpact *s = (SDTImpact*) x->state;
		double p, v, f;
	...
	__declspec(dllexport) SDTInteractor *SDTImpact_new() {
		SDTInteractor *x;
		SDTImpact *s;
	...
	__declspec(dllexport) void SDTImpact_free(SDTInteractor *x) {
		free(x->state);
		SDTInteractor_free(x);
	}

	__declspec(dllexport) void SDTImpact_setStiffness(SDTInteractor *x, double f) {
		((SDTImpact *)x->state)->stiffness = fmax(0.0, f);
	}

	__declspec(dllexport) void SDTImpact_setDissipation(SDTInteractor *x, double f) {
		((SDTImpact *)x->state)->dissipation = fmax(0.0, f);
	}

	__declspec(dllexport) void SDTImpact_setShape(SDTInteractor *x, double f) {
	...
	__declspec(dllexport) double SDTFriction_ElastoPlastic(SDTInteractor *x) {
	...
	__declspec(dllexport) SDTInteractor *SDTFriction_new() {
		SDTInteractor *x;
		SDTFriction *s;
	...
	__declspec(dllexport) void SDTFriction_free(SDTInteractor *x) {
		free(x->state);
		SDTInteractor_free(x);
	}

	__declspec(dllexport) void SDTFriction_setNormalForce(SDTInteractor *x, double f) {
		SDTFriction *s = (SDTFriction *)x->state;
		s->fn = fmax(0.0, f);
		s->fs = s->fn * s->ks;
		s->fc = s->fn * s->kd;
	...
	__declspec(dllexport) void SDTFriction_setStribeckVelocity(SDTInteractor *x, double f) {
		((SDTFriction *)x->state)->vs = fmax(0.0, f);
	}

	__declspec(dllexport) void SDTFriction_setStaticCoefficient(SDTInteractor *x, double f) {
		SDTFriction *s = (SDTFriction *)x->state;
		s->ks = SDT_fclip(f, 0.0, 1.0);
		s->fs = s->fn * s->ks;
	}

	__declspec(dllexport) void SDTFriction_setDynamicCoefficient(SDTInteractor *x, double f) {
		SDTFriction *s = (SDTFriction *)x->state;
		s->kd = SDT_fclip(f, 0.0, 1.0);
		s->fc = s->fn * s->kd;
	}

	__declspec(dllexport) void SDTFriction_setBreakAway(SDTInteractor *x, double f) {
		((SDTFriction *)x->state)->kba = SDT_fclip(f, 0.0, 1.0);
	}

	__declspec(dllexport) void SDTFriction_setStiffness(SDTInteractor *x, double f) {
		((SDTFriction *)x->state)->s0 = fmax(0.0, f);
	}

	__declspec(dllexport) void SDTFriction_setDissipation(SDTInteractor *x, double f) {
		((SDTFriction *)x->state)->s1 = fmax(0.0, f);
	}

	__declspec(dllexport) void SDTFriction_setViscosity(SDTInteractor *x, double f) {
		((SDTFriction *)x->state)->s2 = fmax(0.0, f);
	}

	__declspec(dllexport) void SDTFriction_setNoisiness(SDTInteractor *x, double f) {
