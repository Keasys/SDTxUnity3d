...
#define MAX_RATE   100000.0
	...
	__declspec(dllexport) SDTBubble *SDTBubble_new() {
		SDTBubble *x;
	...
	__declspec(dllexport) void SDTBubble_free(SDTBubble *x) {
		free(x);
	}

	__declspec(dllexport) void SDTBubble_setRadius(SDTBubble *x, double f) {
		x->radius = SDT_fclip(f, MIN_RADIUS, MAX_RADIUS);
	}

	__declspec(dllexport) void SDTBubble_setDepth(SDTBubble *x, double f) {
		x->depth = SDT_fclip(f, 0.0, 1.0);
	}

	__declspec(dllexport) void SDTBubble_setRiseFactor(SDTBubble *x, double f) {
		x->riseFactor = SDT_fclip(f, 0.0, MAX_RISE);
	}

	__declspec(dllexport) void SDTBubble_update(SDTBubble *x) {
		double pRadius;
	...
	__declspec(dllexport) void SDTBubble_normAmp(SDTBubble *x) {
		x->amp = 1.0;
	}

	__declspec(dllexport) double SDTBubble_dsp(SDTBubble *x) {
		double alpha;
	...
	__declspec(dllexport) SDTFluidFlow *SDTFluidFlow_new(int nBubbles) {
		SDTFluidFlow *x;
		int i;
	...
	__declspec(dllexport) void SDTFluidFlow_free(SDTFluidFlow *x) {
		int i;
	...
	__declspec(dllexport) void SDTFluidFlow_setMinRadius(SDTFluidFlow *x, double f) {
		x->minRadius = SDT_fclip(f, MIN_RADIUS, x->maxRadius);
	}

	__declspec(dllexport) void SDTFluidFlow_setMaxRadius(SDTFluidFlow *x, double f) {
		x->maxRadius = SDT_fclip(f, x->minRadius, MAX_RADIUS);
		x->gain = MAX_RADIUS / x->maxRadius;
	}

	__declspec(dllexport) void SDTFluidFlow_setExpRadius(SDTFluidFlow *x, double f) {
		x->expRadius = SDT_fclip(f, 0.0, MAX_EXP);
	}

	__declspec(dllexport) void SDTFluidFlow_setMinDepth(SDTFluidFlow *x, double f) {
		x->minDepth = SDT_fclip(f, 0.0, x->maxDepth);
	}

	__declspec(dllexport) void SDTFluidFlow_setMaxDepth(SDTFluidFlow *x, double f) {
		x->maxDepth = SDT_fclip(f, x->minDepth, 1.0);
	}

	__declspec(dllexport) void SDTFluidFlow_setExpDepth(SDTFluidFlow *x, double f) {
		x->expDepth = SDT_fclip(f, 0.0, MAX_EXP);
	}

	__declspec(dllexport) void SDTFluidFlow_setRiseFactor(SDTFluidFlow *x, double f) {
		x->riseFactor = SDT_fclip(f, 0.0, MAX_RISE);
	}

	__declspec(dllexport) void SDTFluidFlow_setRiseCutoff(SDTFluidFlow *x, double f) {
		x->riseCutoff = SDT_fclip(f, 0.0, 1.0);
	}

	__declspec(dllexport) void SDTFluidFlow_setAvgRate(SDTFluidFlow *x, double f) {
		x->avgRate = SDT_fclip(f, 0.0, MAX_RATE);
		x->success = x->avgRate * SDT_timeStep;
	}

	__declspec(dllexport) double SDTFluidFlow_dsp(SDTFluidFlow *x) {
		SDTBubble *bubble;
