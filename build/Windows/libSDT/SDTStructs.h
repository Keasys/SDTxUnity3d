...
extern __declspec(dllexport) SDTHashmap *SDTHashmap_new(int size);

/** @brief Object destructor.
@param[in] x pointer to the instance to destroy */
extern __declspec(dllexport) void SDTHashmap_free(SDTHashmap *x);
...
extern __declspec(dllexport) void *SDTHashmap_get(SDTHashmap *x, char *key);
...
extern __declspec(dllexport) int SDTHashmap_put(SDTHashmap *x, char *key, void *value);

/** @brief Deletes a key/value pair from the hashmap.
@param[in] key Key to look for in the hashmap
@return 0 if deletion is succesful, 1 otherwise (e.g. key not found) */
extern __declspec(dllexport) int SDTHashmap_del(SDTHashmap *x, char *key);

/** @brief Deletes all the entries in the hashmap. */
extern __declspec(dllexport) void SDTHashmap_clear(SDTHashmap *x);
