...
extern "C" {
	...
	__declspec(dllexport) SDTReverb *SDTReverb_new(long maxDelay) {
		SDTReverb *x;
		int i;
	...
	__declspec(dllexport) void SDTReverb_free(SDTReverb *x) {
		int i;
	...
	__declspec(dllexport) void SDTReverb_update(SDTReverb *x) {
	...
	__declspec(dllexport) void SDTReverb_setXSize(SDTReverb *x, double f) {
		x->xSize = fmax(0.0, f);
	}

	__declspec(dllexport)  void SDTReverb_setYSize(SDTReverb *x, double f) {
		x->ySize = fmax(0.0, f);
	}

	__declspec(dllexport) void SDTReverb_setZSize(SDTReverb *x, double f) {
		x->zSize = fmax(0.0, f);
	}

	__declspec(dllexport) void SDTReverb_setRandomness(SDTReverb *x, double f) {
		x->randomness = SDT_fclip(f, 0.0, 1.0);
	}

	__declspec(dllexport) void SDTReverb_setTime(SDTReverb *x, double f) {
		x->time = fmax(0.0, f);
	}

	__declspec(dllexport) void SDTReverb_setTime1k(SDTReverb *x, double f) {
		x->time1k = fmax(0.0, f);
	}

	__declspec(dllexport) double SDTReverb_dsp(SDTReverb *x, double in) {
	...
	__declspec(dllexport) SDTPitchShift *SDTPitchShift_new(int size, int oversample) {
		SDTPitchShift *x;
	...
	__declspec(dllexport) void SDTPitchShift_free(SDTPitchShift *x) {
		free(x->buf);
		free(x->win);
		free(x->dWin);
		free(x->out);
	...
	__declspec(dllexport) void SDTPitchShift_setRatio(SDTPitchShift *x, double f) {
		x->ratio = fmax(f, 0.0);
	}

	__declspec(dllexport) void SDTPitchShift_setOverlap(SDTPitchShift *x, double f) {
		x->hopSize = SDT_fclip(x->size * (1.0 - f), 1.0, x->size);
		x->gain = 4.0 * x->hopSize / (SDT_SQRT2 * x->size);
	}

	__declspec(dllexport) double SDTPitchShift_dsp(SDTPitchShift *x, double in) {
