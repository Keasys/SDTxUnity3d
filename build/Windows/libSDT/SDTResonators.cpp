...
#define MAX_POS 10000.0
	...
	__declspec(dllexport) double modalPosition(SDTResonator *x, unsigned int mode, double f) {
		return SDT_fclip(x->b1[mode] * f - x->a1[mode] * x->p0[mode] - x->a2[mode] * x->p1[mode], -MAX_POS, MAX_POS);
	}

	__declspec(dllexport) double modalVelocity(SDTResonator *x, unsigned int mode, double p) {
		return x->b0v[mode] * p + x->b1v[mode] * x->p0[mode];
	}

	__declspec(dllexport) double modalEnergy(SDTResonator *x, unsigned int mode, double p, double v) {
		return 0.5 * (x->k[mode] * p * p + x->m[mode] * v * v);
	}

	__declspec(dllexport) void distributeForce(SDTResonator *x, unsigned int pickup, double *fs, double f) {
		int mode;
	...
	__declspec(dllexport) void updateState(SDTResonator *x, unsigned int mode) {
		x->p1[mode] = (x->v[mode] - x->b0v[mode] * x->p0[mode]) / x->b1v[mode];
	}

	__declspec(dllexport) void updateMode(SDTResonator *x, unsigned int mode) {
	...
	__declspec(dllexport) void updatePickup(SDTResonator *x, unsigned int pickup) {
		int mode;
	...
	__declspec(dllexport) void updateModes(SDTResonator *x) {
		int mode;
	...
	__declspec(dllexport) void updatePickups(SDTResonator *x) {
		int pickup;
	...
	__declspec(dllexport) void updateAll(SDTResonator *x) {
		updateModes(x);
		updatePickups(x);
	}

	__declspec(dllexport) SDTResonator *SDTResonator_new(unsigned int nModes, unsigned int nPickups) {
		SDTResonator *x;
		int pickup, mode;
	...
	__declspec(dllexport) void SDTResonator_free(SDTResonator *x) {
		int pickup;
	...
	__declspec(dllexport) double SDTResonator_getPosition(SDTResonator *x, unsigned int pickup) {
		double out;
		int mode;
	...
	__declspec(dllexport) double SDTResonator_getVelocity(SDTResonator *x, unsigned int pickup) {
		double out;
		int mode;
	...
	__declspec(dllexport) int SDTResonator_getNPickups(SDTResonator *x) {
		return x->nPickups;
	}

	__declspec(dllexport) void SDTResonator_setPosition(SDTResonator *x, unsigned int pickup, double f) {
		int mode;
	...
	__declspec(dllexport) void SDTResonator_setVelocity(SDTResonator *x, unsigned int pickup, double f) {
		int mode;
	...
	__declspec(dllexport) void SDTResonator_setFrequency(SDTResonator *x, unsigned int mode, double f) {
		if (mode < x->nModes) {
	...
	__declspec(dllexport) void SDTResonator_setDecay(SDTResonator *x, unsigned int mode, double f) {
		if (mode < x->nModes) {
	...
	__declspec(dllexport) void SDTResonator_setWeight(SDTResonator *x, unsigned int mode, double f) {
		if (mode < x->nModes) {
	...
	__declspec(dllexport) void SDTResonator_setGain(SDTResonator *x, unsigned int pickup, unsigned int mode, double f) {
	...
	__declspec(dllexport) void SDTResonator_setFragmentSize(SDTResonator *x, double f) {
		x->fragmentSize = SDT_fclip(f, 0.0, 1.0);
		updateModes(x);
	}

	__declspec(dllexport) void SDTResonator_setActiveModes(SDTResonator *x, unsigned int i) {
		x->activeModes = SDT_clip(i, 0, x->nModes);
		updateAll(x);
	}

	__declspec(dllexport) void SDTResonator_applyForce(SDTResonator *x, unsigned int pickup, double f) {
		double *fs;
		fs=(double*)malloc(sizeof(double)* x->activeModes);
		int mode;
	...
	__declspec(dllexport) double SDTResonator_computeEnergy(SDTResonator *x, unsigned int pickup, double f) {
		double out, *fs, p, v;
		int mode;
	...
	__declspec(dllexport) void SDTResonator_dsp(SDTResonator *x) {
		double p;
		int mode;
