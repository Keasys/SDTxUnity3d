...
extern __declspec(dllexport) SDTReverb *SDTReverb_new(long maxDelay);

/** @brief Object destructor.
@param[in] x Pointer to the instance to destroy */
extern __declspec(dllexport) void SDTReverb_free(SDTReverb *x);

/** @brief Sets the room width.
@param[in] f Room width, in m */
extern __declspec(dllexport) void SDTReverb_setXSize(SDTReverb *x, double f);

/** @brief Sets the room height.
@param[in] f Room height, in m */
extern __declspec(dllexport) void SDTReverb_setYSize(SDTReverb *x, double f);

/** @brief Sets the room depth.
@param[in] f Room depth, in m */
extern __declspec(dllexport) void SDTReverb_setZSize(SDTReverb *x, double f);
...
extern __declspec(dllexport) void SDTReverb_setRandomness(SDTReverb *x, double f);

/** @brief Sets the global, frequency-independent reverberation time.
@param[in] f Reverberation time, in s */
extern __declspec(dllexport) void SDTReverb_setTime(SDTReverb *x, double f);

/** @brief Sets the reverberation time at 1kHz.
@param[in] f Reverberation time at 1kHz, in s */
extern __declspec(dllexport) void SDTReverb_setTime1k(SDTReverb *x, double f);

/** @brief Updates the internal filters.
Call this function after every sample rate change. */
extern __declspec(dllexport) void SDTReverb_update(SDTReverb *x);
...
extern __declspec(dllexport) double SDTReverb_dsp(SDTReverb *x, double in);
...
typedef __declspec(dllexport) struct SDTPitchShift SDTPitchShift;

/** @brief Object constructor.
@param[in] size Internal buffer size, in samples
@param[in] oversample FFT oversampling rate
@return Pointer to the new instance */
extern __declspec(dllexport) SDTPitchShift *SDTPitchShift_new(int size, int oversample);

/** @brief Object destructor.
@param[in] x Pointer to the instance to destroy */
extern __declspec(dllexport) void SDTPitchShift_free(SDTPitchShift *x);

/** @brief Sets the pitch shifting ratio.
@param[in] f New pitch / original pitch ratio */
extern __declspec(dllexport) void SDTPitchShift_setRatio(SDTPitchShift *x, double f);
...
extern __declspec(dllexport) void SDTPitchShift_setOverlap(SDTPitchShift *x, double f);

/** @brief Signal processing routine.
Call this function at sample rate to compute the pitch shifted signal.
@param[in] in Input sample
@return Output sample */
extern __declspec(dllexport) double SDTPitchShift_dsp(SDTPitchShift *x, double in);
